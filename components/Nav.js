...
function Nav(){
    ...
    return(
        <nav className="w-full inline-block">
            <div className="fixed top-0 left-0 px-4 py-2 border-t-4 border-b-2 bg-black text-white z-20 border-black max-w-[1280px]"><ReactAudioPlayer src="audio/encroaching.mp3" id="audioAmbient" volume={0.09} loop/>{isPlaying ? <AiFillPauseCircle onClick={stopMusic} className="inline text-3xl hover:cursor-pointer"/> : <AiFillPlayCircle onClick={stopMusic} className="inline text-3xl hover:cursor-pointer"/>} <p className="inline text-xs font-semibold text-white uppercase">Toggle music</p></div>
